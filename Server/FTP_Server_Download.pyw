...
def ngrok():
	porta = port.get()
	ngrokStatus = os.popen('start ngrok tcp %s'%porta).read()
	print(ngrokStatus)
...
start_ftp = (lambda: threading._start_new_thread(ftp,()))
send = Button(f_buttons,text="Start FTP ",font=('Verdana',10,'bold'),padx=10, borderwidth=4, command=start_ftp); send.grid(row=1,column=0)

start_ngrok = (lambda: threading._start_new_thread(ngrok,()))
botao_ngrok = Button(f_buttons,text="Start Ngrok",font=('Verdana',10,'bold'),padx=10,borderwidth=4,command=start_ngrok); botao_ngrok.grid(row=1,column=1)
